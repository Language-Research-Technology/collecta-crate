...
class TinyCrate:
    def __init__(self, jsonld=None, directory=None, token=None):
...
class TinyEntity:
    ...
    def _fetch_http(self):
        try:
            if self.crate.token is not None:
                with requests.get(self.id, stream=True, headers={'Authorization': 'Bearer ' + self.crate.token}) as response:
                ...
                with requests.get(self.id, stream=True) as response:
                    if response.ok:
                        return response.text
                    raise TinyCrateException(
                        f"http request to {self.id} failed with status {response.status_code}"
